...
# opencv video work
  ...
  fourcc = cv2.VideoWriter_fourcc(*'MPEG')  #cv2.cv.CV_FOURCC('M', 'P', '4', 'A') #   cv2.cv.CV_FOURCC(*'MPEG') #
  # out = cv2.VideoWriter('saved_videos/126.mp4', fourcc, 20.0, (640, 480))
  # video_filename = 'saved_videos/' + str(time.time()) + '.mp4'
...
# cap = cv2.VideoCapture(0)
  while(cap.isOpened()):
    ...
    # lower_range = np.array([0, 120, 0])
    # upper_range = np.array([20, 156, 62])
